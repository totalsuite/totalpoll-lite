...
use TotalPollVendors\TotalCore\Contracts\Modules\Manager as ManagerContract;
...
class Manager implements ManagerContract {
	...
	public function uninstall( $moduleId ) {
		...
		return new \WP_Error( 'uninstall_failure', __( 'Unable to uninstall this module. Refresh this page if the problem persists.', \TotalPollVendors\TotalCore\Application::getInstance()->env( 'slug' ) ) );
	...
	public function install( $moduleZip ) {
		...
		if ( $moduleZip instanceof FileContract ):
			...
			if ( $result === true ):
				...
				if ( empty( $dirs ) ):
					// Nothing to install
					$result = new \WP_Error( 'empty_archive', __( 'Archive is empty or does not contain a directory.', \TotalPollVendors\TotalCore\Application::getInstance()->env( 'slug' ) ) );
				else:
					// Get first directory
					$moduleDir = $dirs[0];

					if ( ! file_exists( "{$moduleDir}/module.json" ) ):
						$result = new \WP_Error( 'no_module_json', __( 'module.json file is absent.', \TotalPollVendors\TotalCore\Application::getInstance()->env( 'slug' ) ) );
					else:
						// Parse attributes
	...
	public function activate( $moduleId ) {
		...
		return new \WP_Error( 'activation_failure', __( 'Unable to activate this module. Refresh this page if the problem persists.', \TotalPollVendors\TotalCore\Application::getInstance()->env( 'slug' ) ) );
	}
	...
	public function deactivate( $moduleId ) {
		...
		return new \WP_Error( 'deactivation_failure', __( 'Unable to deactivate this module. Refresh this page if the problem persists.', \TotalPollVendors\TotalCore\Application::getInstance()->env( 'slug' ) ) );
	}
