...
class Database {
	...
	public function __construct() {
		$this->optionName = \TotalPollVendors\TotalCore\Application::getInstance()->env( 'prefix' ) . 'migration_last_filename';

		// Migrations files
		$migrations = glob( \TotalPollVendors\TotalCore\Application::getInstance()->env( 'db.migrations' ) . '/*_*_*_*_*.sql' );
		// Last file
	...
	public function upgrade() {
		...
		foreach ( $this->sqlStatements as $sqlStatement ):
			$sqlStatement = Strings::template( (string) $sqlStatement, [ 'db' => \TotalPollVendors\TotalCore\Application::getInstance()->env( 'db' ) ] );

			if ( ! empty( $sqlStatement ) ):
				\TotalPollVendors\TotalCore\Application::get( 'database' )->query( $sqlStatement );
			endif;
		endforeach;
