...
use TotalPollVendors\TotalCore\Contracts\Form\Page as PageContract;
use TotalPollVendors\TotalCore\Helpers\Arrays;
use TotalPollVendors\TotalCore\Http\MimeTypes;

/**
 * Class Field
 * @package TotalPollVendors\TotalCore\Form
 */
abstract class Field implements \TotalPollVendors\TotalCore\Contracts\Form\Field {
	...
	public function validate( $rules = [] ) {
		...
		foreach ( (array) $rules as $rule => $ruleArgs ):
			...
			elseif ( is_string( $rule ) && method_exists( '\TotalPollVendors\TotalCore\Form\Validator', "is{$rule}" ) ):
				$callback = [ '\TotalPollVendors\TotalCore\Form\Validator', "is{$rule}" ];
			endif;
			...
			if ( is_callable( $callback ) ):
				...
				if ( $returnedValue !== true ):
					...
					$this->errors[ $rule ] = str_replace( [ '{{label}}' ], [ $this->getOption( 'label', __( 'This field', \TotalPollVendors\TotalCore\Application::getInstance()->env( 'slug' ) ), true ) ], $returnedValue );
				...
				$errors = array_map(
					function ( $item ) {
						return str_replace(
							...
							[
								$item,
								\TotalPollVendors\TotalCore\Application::getInstance()->env( 'slug' ),
							],
							$this->errorTemplate );
					},
					$this->errors
	...
	public function getLabelHtmlElement() {
		...
		$defaults = [
			...
			'class'   => \TotalPollVendors\TotalCore\Application::getInstance()->env( 'slug' ) . '-form-field-label',
		];
		...
		if ( $label ):
			...
			$label = new \TotalPollVendors\TotalCore\Helpers\Html( 'label', $attributes, $content );
		endif;
	...
	public function setOptions( $options ) {
		if ( ! empty( $options['options'] ) && is_string( $options['options'] ) ):
			...
			$options['options'] = (array) explode( PHP_EOL, $options['options'] );
			$optionsArray       = [];
